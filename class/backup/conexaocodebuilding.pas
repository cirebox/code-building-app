...
begin
  case dm.fab.Conexao.SGDB of
    firebird:
    begin
      FInstance.Conexao.Protocol := 'firebird-2.5';
      FInstance.Conexao.LibraryLocation := dm.fab.Dir+'bin\fbclient.dll';
    end;
    mysql   :
    begin
      FInstance.Conexao.Protocol := 'mysql-5';
      FInstance.Conexao.LibraryLocation := dm.fab.Dir+'bin\libmysql.dll';
    end;
    postgres:
    begin
      FInstance.Conexao.Protocol         := 'postgresql';
      FInstance.Conexao.LibraryLocation  := dm.fab.Dir+'bin\libpq.dll';
    end;
  end;
  FInstance.Conexao.AutoEncodeStrings := False;
  FInstance.Conexao.LoginPrompt       := False;
  FInstance.Conexao.HostName          := dm.fab.Conexao.Host;
  FInstance.Conexao.Port              := dm.fab.Conexao.Porta;
  FInstance.Conexao.Password          := dm.fab.Conexao.Senha;
  FInstance.Conexao.User              := dm.fab.Conexao.Usuario;
  FInstance.Conexao.Database          := dm.fab.Conexao.Banco;
end;
...
var
  qry: TZQuery;
begin
  if dm.fab.Conexao.SGDB = firebird then
    Exit;
  try
    try
...
begin
  try
    try
      ...
      Script.SaveToFile(dm.fab.DirSaida+'teste'+'.TXT');
